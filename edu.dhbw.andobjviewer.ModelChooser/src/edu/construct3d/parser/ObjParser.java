...
public class ObjParser {
	...
	public Model parse(String modelName, BufferedReader is) throws IOException, ParseException {
		...
		String line;
		int lineNum = 1;
		int countvertex=0,counttexture=0,countnormal=0;
		...
		line = is.readLine(), lineNum++)
		{
			if (line.length() > 0) {
				if (line.startsWith("#")) {
					...
					String temp1=spaceTokenizer.next();
					String temp2=spaceTokenizer.next();
					String temp3=spaceTokenizer.next();
					countvertex++;
					vertices.add(new float[]{
							
							Float.parseFloat(temp1),
							Float.parseFloat(temp2),
							Float.parseFloat(temp3)});
				}
					...
					for (int i = 0; i < 3; i++) {//only triangles supported
						...
						if(vertexCount == 2) {
							...
							vertexID = Integer.parseInt(slashTokenizer.next())-1;
							if(vertexID<0)
								vertexID=vertexID+countvertex+1;
							...
							if(!texCoord.equals("")) {
								...
								if(textureID<0)
								{
									textureID=textureID+counttexture+1;
								}
							}
							//normal reference
							normalID = Integer.parseInt(slashTokenizer.next())-1;
							if(normalID<0)
							{
								normalID=normalID+countnormal+1;
							}
						} else {
							throw new ParseException(modelName,
									lineNum,
